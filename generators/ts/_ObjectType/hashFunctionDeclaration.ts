...
export function hashFunctionDeclaration(
  ...
  return {
    ...
    parameters: [
      {
        name: thisVariable,
        type: `Omit<${this.interfaceQualifiedName}, "${this.configuration.objectTypeIdentifierPropertyName}"> & { ${this.configuration.objectTypeIdentifierPropertyName}?: ${this.identifierType.name} }`,
      },
      {
        name: hasherVariable,
        type: "HasherT",
